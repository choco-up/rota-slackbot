...
const utils = {
	regex: {
		...
		staff:
			/^<@(U[A-Z0-9]+?)> "([A-Za-z0-9\-\s]+?)" (staff) (<@U[<@>A-Z0-9,\s]+?>)$/g,
		// @rota "[rotation]" reset staff
		// Removes rotation staff list
		"reset staff": /^<@(U[A-Z0-9]+?)> "([A-Za-z0-9\-\s]+?)" (reset staff)$/g,
		...
		assign:
			/^<@(U[A-Z0-9]+?)> "([A-Za-z0-9\-\s]+?)" (assign) (<@U[A-Z0-9]+?>)(.*)$/g,
		// @rota "[rotation]" assign next [optional handoff message]
		// Assigns a user to a rotation
		"assign next":
			/^<@(U[A-Z0-9]+?)> "([A-Za-z0-9\-\s]+?)" (assign next)(.*)$/g,
		// @rota "[rotation]" who
		// Responds stating who is on-call for a rotation
		who: /^<@(U[A-Z0-9]+?)> "([A-Za-z0-9\-\s]+?)" (who)$/g,
		// @rota "[rotation]" tag 
		// Tag the person who is on-call for a rotation, in the same thread
		tt: /^<@(U[A-Z0-9]+?)> "([A-Za-z0-9\-\s]+?)" (tt)$/g,
		// @rota "[rotation]" about
		...
		about: /^<@(U[A-Z0-9]+?)> "([A-Za-z0-9\-\s]+?)" (about)$/g,
		// @rota "[rotation]" unassign
		// Unassigns rotation
		unassign: /^<@(U[A-Z0-9]+?)> "([A-Za-z0-9\-\s]+?)" (unassign)$/g,
		// @rota delete "[rotation]"
		// Removes the rotation completely
		delete: /^<@(U[A-Z0-9]+?)> (delete) "([A-Za-z0-9\-\s]+?)"$/g,
		// @rota help
		// Post help messaging
		...
		// Sends message text
		message: /^<@(U[A-Z0-9]+?)> "([A-Za-z0-9\-\s]+?)" (.*)$/g,
	},
	/**
	...
	isCmd(cmd, input) {
		...
		for (const line of splitByNewline) {
			const msg = utils.cleanText(line);
	...
	async parseCmd(cmd, e, ct) {
		const splitByNewline = e.text.split("\n");
		for (const line of splitByNewline) {
			const cleanText = utils.cleanText(line);
			const regex = new RegExp(utils.regex[cmd]);
	 ...
	 */
	msgConfig(botToken, channelID, text, ts = null) {
		return {
			token: botToken,
			channel: channelID,
			text: text,
			thread_ts: ts?.toString(),
		};
	},
	/**
